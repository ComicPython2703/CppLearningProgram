...
> 在 `cout` 中使用三目运算符时，给表达式必须加括号 `()`，因为 `<<` 的优先级高于 `?:`，不加括号会导致只输出0,也就是`false`
...
- `system_clock` 一个结构体
- `time_t` 是C/C++标准时间类型,表示从1970/1/1 00:00:00 UTC 到现在的精确秒数;
...
struct tm
{
    ...
    int tm_year;  // 从1990年开始,32位系统最大2038!!! 
...
> **`int tm_year;` 这段,32位系统最大2038!!!**

- `tm_year`需要加1990才能显示正常年份
- `tm_mon`需要加1,因为是从0开始
...
```
 ...
 在这段代码,Windows和Linux主要区别是`localtime_`后面接的字母.
 windows是`s`,linux是`r`
